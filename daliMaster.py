...
class daliMaster:
    ...
    def begin(self, toSet = defines.LW14_I2C_ADDRESS):
        ...
        try :
            self.__light_sensor_address = defines.BH_1750_I2C_ADDRESS
    ...
    def read(self, regName):
        ...
        try:
            ...
            if regName == "status" :
                ...
                read = self.__status_reg_as_dict(data=read)
                return read
        except ValueError as e:
            print(e.args)
            return defines.ERROR
    ...
    def light_in_lux_data(self):

        # data = self.__i2cRead_light_level(defines.ONE_TIME_HIGH_RES_MODE_1)
        data = self.__i2cRead_light_level(defines.CONTINUOUS_HIGH_RES_MODE_1)
        # time.sleep(0.18)
        # data = self.__i2cRead_light_level(defines.ONE_TIME_HIGH_RES_MODE_1)
        # time.sleep(0.5)
        lux_data = (data[1] + (256 * data[0])) / 1.2
        return (lux_data)
    ...
    def __i2cRead_light_level(self, bh1750_mode):

        return self.__bus.read_i2c_block_data(self.__light_sensor_address, bh1750_mode)
    ...
    def __status_reg_as_dict(self, data):
        bits_desc = ["1 BYTE TELEGRAM", "2 BYTE TELEGRAM", "REPLY TIMEFRAME", "VALID REPLY", "FRAM ERROR", "OVERRUN", "BUSY", "BUS FAULT"]
        bits_desc.reverse()
        reg = bin(data)[2:]
        if data == 0: reg='00000000'
        reg = list(reg)
        # print(reg)
        status_dict = {}
        for key in bits_desc:
            for value in reg:
